...
A responsive two-player Tictactoe project. The former plain-JavaScript frontend has been replaced by an empty React/Vite migration scaffold; the supplied Java servlet server stores rooms and boards in memory.

The React modules are intentionally empty so the application can be migrated feature by feature. See [REACT-README.md](REACT-README.md) for the architecture, setup instructions, Vite commands, and recommended implementation order.
